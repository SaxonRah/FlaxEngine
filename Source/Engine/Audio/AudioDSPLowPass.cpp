...
AudioDSPLowPass::AudioDSPLowPass()
    : AudioDSPEffect(EffectType::LowPass)
    , _cutoffFrequency(1000.0f)
    , _resonance(0.707f)
    , _a0(0)
    , _a1(0)
    , _a2(0)
...
void AudioDSPLowPass::SetCutoffFrequency(float value)
{
    ScopeLock lock(_locker);
    _cutoffFrequency = Math::Clamp(value, 20.0f, 20000.0f);
}

void AudioDSPLowPass::SetResonance(float value)
{
    ScopeLock lock(_locker);
    _resonance = Math::Clamp(value, 0.1f, 10.0f);
}

void AudioDSPLowPass::Process(const float* input, float* output, int32 sampleCount, int32 channels, int32 sampleRate)
{
    if (!input || !output)
    {
        LOG(Error, "AudioDSPLowPass: Null input or output pointer");
        return;
    }
    ...
    {
        LOG(Warning, "AudioDSPLowPass: Sample count too large ({0}), skipping processing", sampleCount);
...
void AudioDSPLowPass::UpdateCoefficients(int32 sampleRate)
{
    ...
    // Biquad filter coefficients for low-pass
